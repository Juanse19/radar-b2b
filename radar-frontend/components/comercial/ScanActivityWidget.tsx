...
import type { ActiveScan, ScanEvent } from '@/lib/comercial/scan-activity-store';
    ...
    >
      ...
      <div className="flex items-center justify-between border-b border-border px-4 py-2.5">
        ...
        <div className="flex items-center gap-2">
          {activeScan && (
            <Link
              href={`/vivo?sessionId=${activeScan.sessionId}&line=${encodeURIComponent(activeScan.line)}&provider=${activeScan.provider}&empresas=${encodeURIComponent(JSON.stringify(activeScan.empresas.map(name => ({ name, country: '' }))))}`}
