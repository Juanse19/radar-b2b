 ...
 *
 * Contract: the LLM returns categorical strings per dimension (NOT numbers).
 * The backend maps each `valor` to a 0–10 score via `categoricoToScore` in
 * `scoring.ts`. This keeps the LLM output human-auditable and the scoring
 * deterministic.
 */
...
Tu misión: clasificar cada empresa en 9 dimensiones basándote en su perfil público y presencia en el mercado.
...
"> 5M USD":     proyecto mayor, multiplanta, multinacional
"1-5M USD":     proyecto grande, empresa regional
"500K-1M USD":  proyecto medio, empresa mediana
"< 500K USD":   proyecto pequeño, empresa local
"Sin ticket":   empresa muy pequeña o sin datos
...
━━ TIER ━━
"A": referente internacional + presencia internacional + impacto Alto/Muy Alto + prioridad comercial Alta/Muy Alta
"B": referente país + sedes regionales + impacto Medio/Alto + prioridad Media/Alta
"C": baja visibilidad o única sede + impacto Bajo/Muy Bajo + prioridad Baja/Muy Baja
...
${ragBlock}
━━ INSTRUCCIÓN ━━
Perfilá la empresa con búsqueda web (empleados, plantas, países, sector exacto, capacidades, noticias de inversión). Luego clasificá las 9 dimensiones según la rúbrica del system prompt.
