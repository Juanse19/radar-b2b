...
interface Props {
  line:     string;
  selected: RadarV2Company[];
  onChange: (companies: RadarV2Company[]) => void;
  maxSelect?: number;
}

export function CompanySelector({ line, selected, onChange, maxSelect = 20 }: Props) {
  const [all, setAll]         = useState<RadarV2Company[]>([]);
  ...
  const fetchCompanies = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/radar-v2/companies?${params}`);
