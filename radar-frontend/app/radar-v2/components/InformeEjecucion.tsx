...
import type { Informe, VerificationFlag } from '@/lib/radar-v2/types';
...
export function InformeEjecucion({ sessionId, open, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/radar-v2/reports/${sessionId}`)
      .then(res => {
        ...
        <DialogFooter className="flex-row gap-2">
          {informe && (
            <Button
              variant="outline"
              size="sm"
              onClick={() => window.open(`/api/radar-v2/reports/${sessionId}?format=md`)}
            >
