...
import { buildExcelWorkbook, type ExcelSessionData } from '@/lib/radar-v2/excel-builder';
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const filename = `radar-v2-${s.linea_negocio.replace(/[^a-zA-Z0-9]/g, '-')}-${new Date(s.created_at).toISOString().slice(0, 10)}.xlsx`;
    ...
    console.error('[/api/radar-v2/export] Error:', msg);
