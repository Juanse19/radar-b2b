...
import type { ComercialMetrics } from '@/lib/comercial/types';
...
export default function MetricasV2Page() {
  ...
  const [metrics, setMetrics] = useState<ComercialMetrics | null>(null);
  ...
  const fetchMetrics = useCallback(async (r: Range) => {
    ...
    setError(false);
    try {
      const res = await fetch(`/api/comercial/metrics?range=${r}`);
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      const data = await res.json() as ComercialMetrics;
      setMetrics(data);
    } catch {
      setError(true);
      setMetrics(null);
      ...
      {!loading && error && (
        <div className="rounded-lg border border-destructive/30 bg-destructive/5 py-12 px-6 text-center">
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            Verifica que el endpoint <code className="rounded bg-muted px-1">/api/comercial/metrics</code> esté disponible.
          </p>
          <button
