...
└────────────────┬────────────────────────────────────┘
                 │
                 │ POST /api/comercial/stream (SSE)
                 ▼
┌─────────────────────────────────────────────────────┐
│ API Route: /api/comercial/stream                    │
├─────────────────────────────────────────────────────┤
...
**Estado:**
- URL-driven: `/escanear?mode=manual&line=BHS&companies=dhl,fedex`
...
- GET `/api/comercial/estimate` para cálculo de costo
- POST `/api/comercial/stream` para iniciar escaneo
...
**Características:**
- SSE (Server-Sent Events) desde `/api/comercial/stream`
...
- **Stop button**: POST `/api/comercial/cancel?sessionId=X` (cancela mid-stream)
...
- Integración en `/resultados` como pestaña adicional
...
- Throttle en `/api/comercial/stream` si presupuesto agotado
...
- Se integra como pestaña adicional en `/resultados` ("Calificación")
- Reutiliza misma sesión de scan

**Flujo:**
```
/comercial/resultados?sessionId=X
...
Integración de **WF03 N8N** (búsqueda de contactos Apollo.io) en `/prospectos`:
...
Estos se validan en:
- `POST /api/comercial/stream` (before loop)
- `GET /api/comercial/estimate` (cost calculation)
...
| `app/api/comercial/stream/route.ts` | Orquestación SSE principal |
| `app/api/comercial/cancel/route.ts` | Cancelación de sesiones |
| `app/api/admin/tokens/route.ts` | Dashboard de métricas |
| `app/(dashboard)/comercial/escanear/page.tsx` | Wizard UI |
| `app/(dashboard)/comercial/vivo/page.tsx` | Timeline SSE |
| `app/(dashboard)/comercial/resultados/page.tsx` | Tabla histórica |
