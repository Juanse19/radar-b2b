/**
 * Unit tests for lib/comercial/scanner.ts — resolveProviderConfig env-var fallback
...
async function freshScanner(): Promise<{
  mod: typeof import('@/lib/comercial/scanner');
  pgFirst: ReturnType<typeof vi.fn>;
}> {
  vi.resetModules();
  vi.clearAllMocks();
  const mod    = await import('@/lib/comercial/scanner');
